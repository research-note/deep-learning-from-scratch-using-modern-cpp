...
template<typename T>
void MnistParser::setFilePath(string filePath) {
...
void MnistParser::readMnist(char *file_path, int num_data, int len_info, int arr_n, unsigned char data_char[][arr_n], int info_arr[])
{
    int i, j, k, fd;
    unsigned char *ptr;
...
{
    this.readMnist(TRAIN_IMAGE, NUM_TRAIN, LEN_INFO_IMAGE, SIZE, train_image_char, info_image);
